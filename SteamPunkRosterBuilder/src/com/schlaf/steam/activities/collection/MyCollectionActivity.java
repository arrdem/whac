...
public class MyCollectionActivity extends SherlockFragmentActivity implements OnItemSelectedListener {
	...
	public void onItemSelected(AdapterView<?> parent, View view, int position,
		...
		if (parent.getId() == factionSpinner.getId()) {
			if (factionSpinner.getSelectedItemPosition() != Spinner.INVALID_POSITION) {
				...
				ArrayAdapter<ModelTypeEnum> adapterEntryType = new ArrayAdapter<ModelTypeEnum>(this, android.R.layout.simple_spinner_item, android.R.id.text1, types);
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		SharedPreferences save = getSharedPreferences("collection", MODE_PRIVATE);
		...
		List<ModelTypeEnum> entryTypeList = CollectionSingleton.getInstance().getNonEmptyEntryType();
		ArrayAdapter<ModelTypeEnum> adapterEntryType = new ArrayAdapter<ModelTypeEnum>(this, android.R.layout.simple_spinner_item, android.R.id.text1, entryTypeList);
		adapterEntryType.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
		entryTypeSpinner.setAdapter(adapterEntryType);
		...
		// reselect same type if possible
		if (CollectionSingleton.getInstance().getEntryType() == null) {
			CollectionSingleton.getInstance().setEntryType(entryTypeList.get(0));	
		} else {
			int selected = entryTypeList.indexOf(CollectionSingleton.getInstance().getEntryType());
			if (selected == -1) {
				// if type has disappeared, select first by default...
				selected = 0;
			}
 ...
 public boolean onCreateOptionsMenu(Menu menu) {
     ...
     getSupportMenuInflater().inflate(R.menu.collection_menu, menu);
	...
	private void showStats() {
		...
		StorageManager.generateCollectionText(ps);
		ps.close();
		try {
			bos.close();
		} catch (IOException e) {
	...
	private void exportByMail() {
        ...
        emailIntent.putExtra(android.content.Intent.EXTRA_SUBJECT, "WHAC collection export file");
        emailIntent.putExtra(Intent.EXTRA_STREAM, Uri.parse("file://" + strFile));
        emailIntent.putExtra(android.content.Intent.EXTRA_TEXT, "this is the export of your collection");
