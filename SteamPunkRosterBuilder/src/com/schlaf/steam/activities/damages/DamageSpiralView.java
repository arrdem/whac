...
public class DamageSpiralView extends DamageBaseView implements ColumnChangeNotifier  {
	...
	/**
	 * pourcentage du rayon dédié à l'affichage
	 */
	private static final float POURCENTAGE_RAYON = 0.75f;
	/**
	 * ouverture de la spirale (90°)
	 */
	...
	/**
	 * association d'un cercle de dommage à son centre, permet de trouver où l'on clique
	 * @author S0085289
	 *
	 */
	...
	private void drawBranchesBackground(int rayon, int centerX,
		...
		int radialMultiplicatorSelected = 3;
		...
		for (int i = 0; i < 2; i++) {
			...
			paint.setShader(new RadialGradient(x, y, (int) tailleCercle
	...
	private void drawOuterBranchBackground(double theta, int rayon,
		...
		for (int i = 0; i < maxCircle + 1 ; i ++) { //  +1 car cercle de légende 
			...
			double tailleCercle = (1+ ((double) i/maxCircle/2)) * 0.06d * rayon; // de 6% à 8% du rayon 
			...
			// draw background
			if (isSelected) {
				paint.setShader(new RadialGradient(x, y, (int) tailleCercle * radialMultiplicator, new int[] {shadedColor1 , shadedColor2,shadedColor3 }, null, TileMode.CLAMP));
			} else {
				paint.setShader(new RadialGradient(x, y, (int) tailleCercle * radialMultiplicator, shadedColor1 , shadedColor2, TileMode.CLAMP));
			}
