...
public class DeviceFinder implements Runnable {
	...
	private String TAG = "DeviceFinder";
	public static UUID BT_NETWORK_UUID = BlueToothDatalink.BT_NETWORK_UUID;
	...
	private void doDiscovery() {
		Log.d(TAG, "doDiscovery - search for buddies");
		// If we're already discovering, stop it
		if (ba.isDiscovering()) {
			Log.d(TAG, "doDiscovery - already discovering, cancel current discovery and restart discover process");
			ba.cancelDiscovery();
		}
	...
	public void run() {
		...
		while (discovering) {
			try {
				...
				Thread.sleep(1000);
				discovering = ba.isDiscovering();
			} catch (InterruptedException e) {
				Log.d(TAG, "DeviceFinder sleep interrupted");
			}
		}

		Log.d(TAG, "Discovering is finished - handling the devices");
	...
	/**
	 * Called when bluetooth hardware has finished discovery and Devises should
	 * be handled
	 */
		...
		public void run() {
			...
			for (BluetoothDevice device : devices) {
				...
				if (dm.numberOfNeighbours() + newConnections.size() < BlueToothDatalink.MAX_OUT_DEGREE) {
					...
					if (!dm.exists(device.getAddress())) {
						...
						try {
					        ...
					        // socket.getRemoteDevice().getUuids()
