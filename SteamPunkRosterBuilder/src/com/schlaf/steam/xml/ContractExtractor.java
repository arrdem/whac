...
public class ContractExtractor {
	...
	/**
	 * charge la partie "contracts" du fichier xml à partir du parser positionné
	 * sur le tag de début
	 * 
	 * @param xpp
	 */
	...
	/**
	 * charge la partie "contract" du fichier xml à partir du parser positionné
	 * sur le tag de début
	 * 
	 * @param xpp
	 */
		...
		while (!(eventType == XmlPullParser.END_TAG && BENEFITS_TAG.equals(xpp
			...
			if (eventType == XmlPullParser.START_TAG) {
				if (INGAMEEFFECT_TAG.equals(xpp.getName())) {
					String inGameEffect = xpp.getAttributeValue(null, LABEL_ATTRIBUTE);
